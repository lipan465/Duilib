...
namespace DuiLib
{
	...
	void CContainerUI::ScrollEnd()
	{
		if(!m_bScrollEndAfterPaint) m_bScrollEndAfterPaint = true;
	}
	...
	void CContainerUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			...
			if( pControl->IsFloat() ) {
				...
				if( rcCtrl.right > m_rcItem.right ) rcCtrl.right = m_rcItem.right;
				if( rcCtrl.bottom > m_rcItem.bottom ) rcCtrl.bottom = m_rcItem.bottom;
