...
#define IP_KEEP   3
	...
	void CIPAddressUI::SetIP(LPCTSTR strIP)
	{
#ifdef _UNICODE
		int cchStr = ((int) wcslen(strIP) * 2) + 1;
		LPSTR pstr = (LPSTR) _alloca(cchStr);
		if( pstr != NULL ) ::WideCharToMultiByte(::GetACP(), 0, strIP, -1, pstr, cchStr, NULL, NULL);
		SetIP(ntohl(inet_addr(pstr)));
#else
		SetIP(ntohl(inet_addr(strIP)));
#endif
	}
	...
	void CIPAddressUI::UpdateText()
	{
		...
		else if (m_nIPUpdateFlag == IP_UPDATE)
		{
			...
			_stprintf(szIP, _T("%d.%d.%d.%d"),(m_dwIP>>24)&0xFF,(m_dwIP>>16)&0xFF,(m_dwIP>>8)&0xFF,(m_dwIP)&0xFF);
			SetText(szIP);
		}
	}
	...
	void CIPAddressUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		...
		if( _tcsicmp(pstrName, _T("text")) == 0 ) SetIP(pstrValue);
	}
}
