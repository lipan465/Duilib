...
namespace DuiLib {
	...
	void CListTextElementUI::SetText(int iIndex, LPCTSTR pstrText)
	{
		...
		while( m_aBkColors.GetSize() < pInfo->nColumns ) { m_aBkColors.Add(NULL); }
	...
	void CListTextElementUI::SetColumItemColor(int nColum, DWORD iBKColor)
	{
		m_aBkColors.SetAt(nColum,(LPVOID)iBKColor);
	}
	void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
	{
		...
		for( int i = 0; i < pInfo->nColumns; i++ )
		{
			...
			//draw bk color
			DWORD iBkColor = (DWORD)m_aBkColors.GetAt(i);
			if( IsSelected() ) {
				iBkColor = pInfo->dwSelectedBkColor;
			}
			if( !IsEnabled() ) {
				iBkColor = pInfo->dwDisabledBkColor;
			}
			CRenderEngine::DrawColor(hDC,rcItem,iBkColor);
