...
namespace DuiLib {
	...
	bool CListExUI::OnEditEvent(void* param)
	{
		TEventUI *Msg = (TEventUI *)param;
		if(Msg->Type == UIEVENT_MOUSEENTER)
		{
			m_pEditUI->SetFocus();
		}
		else if(Msg->Type == UIEVENT_KILLFOCUS)
		{
			...
			if (pRowCtrl)
			{
				...
				m_pManager->SendNotify(this, DUI_MSGTYPE_TEXTCHANGED,m_nRow,m_nColum);
			}
			m_pEditUI->SetText(_T(""));
			//重置当前行列
	...
	CEditUI* CListExUI::GetEditUI()
	{
		if (m_pEditUI == NULL)
		{
			...
			LPCTSTR pDefaultAttributes = m_pManager->GetDefaultAttributeList(_T("ListEx_Edit"));
			if( pDefaultAttributes ) {
				m_pEditUI->ApplyAttributeList(pDefaultAttributes);
			}

			Add(m_pEditUI);
			m_pEditUI->OnEvent += MakeDelegate(&CListExUI::OnEditEvent,this);
			//m_pEditUI->SetNumberOnly(true);
		}
		if (m_pComboBoxUI)
		{
			RECT rc = {0,0,0,0};
...
Label_ForeImage:
	...
	void CListTextExtElementUI::SetText(int iIndex, LPCTSTR pstrText)
	{
		...
		TEventUI msg;
		msg.Type = UIEVENT_LISTITEMCHANGE;
		msg.pSender = this;
		msg.lParam = (LPARAM)iIndex;
		msg.wParam = (WPARAM)pstrText;
		if( m_pOwner->OnEvent(&msg) )
		{
			if ( pText )
	...
	void CListTextExtElementUI::SetOwner(CControlUI* pOwner)
	{
		...
		m_pOwner = static_cast<CListExUI*>(pOwner->GetInterface(_T("List")));
	}
