...
namespace DuiLib
{
	...
	CNotifyWnd::CNotifyWnd(CDuiPoint point)
	{
		m_OwnerWnd = GetForegroundWindow();
		m_rcPos.top = point.y;
		m_rcPos.left = point.x - 40;

		m_rcPos.bottom = m_rcPos.top + 40;
		m_rcPos.right = point.x + 140;
		...
		Create(m_OwnerWnd, NULL, WS_POPUPWINDOW, 0, m_rcPos);
		HRGN hg = CreatePolygonRgn(m_PolyPoint,7,WINDING);
		SetWindowRgn(m_hWnd,hg, TRUE);

		LONG styleValue = ::GetWindowLong(*this, GWL_STYLE);
		styleValue &= ~WS_BORDER;
		::SetWindowLong(*this, GWL_STYLE, styleValue | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);
	}
	void CNotifyWnd::Init()
	{
		::ShowWindow(m_hWnd, SW_SHOWNORMAL);
		::SetFocus(m_hWnd);
	}
	LPCTSTR CNotifyWnd::GetWindowClassName() const
	{
		return _T("NotifyWnd");
	}
	...
	LRESULT CNotifyWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		LRESULT lRes = 0;
		BOOL bHandled = TRUE;
		if( uMsg == WM_PAINT )
		{
			...
			CDuiString strText = _T("Can Only Input Number!");
			::RECT rect;
			...
			::DeleteObject(hBrush);
			rect.top += 15;
			::DrawText(hDC, strText.GetData(), -1, &rect, DT_CENTER | DT_EDITCONTROL | DT_WORDBREAK);
